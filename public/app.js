...
async function init() {
    ...
    // KILL LOADING SCREEN IMMEDIATELY
    ...
    if (loadingScreen) {
        ...
        console.error('❌ [INIT] Loading screen element not found!');
    }
    
    // Set initial language
    console.log('🚀 [INIT] Setting language...');
    const langText = document.getElementById('langText');
    if (langText) {
        langText.textContent = currentLang === 'ru' ? 'EN' : 'RU';
    }
    updateUILanguage();
    console.log('✅ [INIT] Language set');
    
    // Check auth (don't care if it fails)
    console.log('🚀 [INIT] Checking auth...');
    checkAuth()
        .then(() => console.log('✅ [INIT] Auth checked'))
        .catch(err => console.warn('⚠️ [INIT] Not authenticated:', err));
    
    // Load threads (show error but don't break)
    console.log('🚀 [INIT] Loading threads...');
    loadThreads()
        .then(() => console.log('✅ [INIT] Threads loaded'))
        .catch(err => {
            ...
            if (container) {
                container.innerHTML = `
                    <div style="text-align: center; padding: 40px; color: #666;">
                        <p>Failed to load threads: ${err.message}</p>
                        <button onclick="location.reload()" style="margin-top: 20px; padding: 10px 20px; background: #ff0000; border: none; color: white; border-radius: 6px; cursor: pointer;">Retry</button>
                    </div>
                `;
            }
        });
...
console.log('📄 [DOM] Script loaded, waiting for DOMContentLoaded...');
document.addEventListener('DOMContentLoaded', () => {
    console.log('📄 [DOM] DOMContentLoaded fired, calling init()...');
    init();
});
...
// Emergency fallback - hide loading after 1 second MAX
setTimeout(() => {
    const loadingScreen = document.getElementById('loadingScreen');
    if (loadingScreen) {
        loadingScreen.style.display = 'none';
